...
/**
  ...
  * 应用关联应用分组查询的dao
  */
class AppsAssociateAppGroupDao {
  ...
  def getWithSql(orgId: String, orgType: String, curAppGroupId: String): String = {
    String.format("WITH TEMP AS (SELECT T.APP_PACKAGE_NAME, T.ORG_ID, T.ORG_TYPE" +
      " FROM T_APP_FILE_LATEST T " +
      "WHERE T.ORG_ID = '%s' AND T.ORG_TYPE = '%s'" +
      " MINUS" +
      " SELECT T.APP_PACKAGE_NAME, A.ORG_ID, A.ORG_TYPE  FROM T_APP_GROUP_APPS T" +
      " JOIN T_APP_GROUP A ON T.APP_GROUP_ID = A.APP_GROUP_ID" +
      " WHERE A.ORG_ID = '%s' AND A.ORG_TYPE = '%s' AND T.APP_GROUP_ID = '%s') ", orgId, orgType, orgId, orgType, curAppGroupId)
  }
